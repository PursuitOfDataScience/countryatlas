...
# The two frames the figure draws, plus the counts quoted in the sentence
# above it. Those counts used to be hard-coded prose sitting next to a figure
# built from a live fetch, so they drifted the moment the World Bank revised
# its coverage -- "42 of 215" had become 37 of 210 by the time anyone checked.
...
# Countries each map actually fills, compared on the ISO spine so the two are
# counted the same way.
iso_filled <- unique(na.omit(by_iso$iso3c[!is.na(by_iso$life_exp)]))
name_filled <- unique(na.omit(countrycode::countrycode(
  unique(by_name$region[!is.na(by_name$life_exp)]),
  "country.name", "iso3c", warn = FALSE)))
n_total <- length(iso_filled)
n_lost <- length(setdiff(iso_filled, name_filled))
```

Join the World Bank's life-expectancy table to `map_data("world")` by country
name and **`r n_lost` of `r n_total` countries silently vanish**: nobody spells
