...
package offers a full vocabulary; this vignette tours the ones that need only
`maps` -- itself a suggested package, so the map chunks are skipped when it is
absent -- and points to the rest, which need `sf` or `biscale` as well.
...
```{r eval = has_maps, fig.alt = "Bubble map: population drawn as proportional circles at country centroids."}
...
```{r eval = has_maps, fig.alt = "Spike map: population drawn as vertical spikes rising from country centroids."}
...
```{r eval = has_maps, fig.alt = "Flow map: great-circle arcs joining four origin-destination country pairs, width by volume."}
od <- data.frame(
...
```{r eval = has_maps, fig.height = 5, fig.alt = "Small multiples: one GDP per capita choropleth panel per continent."}
...
```{r eval = has_maps, fig.alt = "Choropleth of Europe with ISO codes labelled at country centroids."}
