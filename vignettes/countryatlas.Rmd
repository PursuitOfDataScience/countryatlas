...
```{r eval = has_maps, fig.alt = "World choropleth of GDP per capita in quantile bins."}
world_map(mapdf, gdp_per_capita, style = "quantile",
          title = "GDP per capita (quantile bins)")
```

```{r eval = has_maps, fig.alt = "World map with every country coloured by its continent."}
...
```{r fig.height = 5, fig.alt = "Equal-area tile grid: one identically sized tile per country, shaded by life expectancy."}
